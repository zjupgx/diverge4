    ...
    std::vector<result_t> results;
		...
		Gu99Calculator(const std::vector<std::string>& args) : argv(args) {
			...
			for (int i = 0; i < argv.size() - 1; i++) {
				Tree tree;
				if (!tree.load(treeFiles[i])){return;} else{trees.push_back(tree);}
			}
		}
		// Gu99Calculator(){}
		void calculate() {
			...
			if (!preprocess(t_names,trees,sequences)) {return;}
		...
		//get t_names
		const std::vector<std::string>& _t_names() const {
				return t_names;
		}
		//get argv
...
PYBIND11_MODULE(_gu99cpp, m) {
    ...
    py::class_<Gu99Calculator>(m, "Gu99Calculator")
        ...
        .def("calculate", &Gu99Calculator::calculate)
				...
				.def("_t_names", &Gu99Calculator::_t_names);
